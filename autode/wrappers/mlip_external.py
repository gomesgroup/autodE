...
class MLIPCalculation:
    ...
    forces: Optional[List[Tuple[float, float, float]]] = None  # Hartree/Angstrom (negated gateway gradient)
...
# <base>.engrad back. <base> is taken from the descriptor filename; if that does not end in
# .extinp.tmp, from the xyz name inside it. A second argument overrides the output name.
python3 - "$@" << 'PYTHON_EOF'
import sys
import json
import time
